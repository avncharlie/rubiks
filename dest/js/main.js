...
var camera;
var renderer;
var controls; // other globals constants
...
init();
animate();
main(); //var axesHelper = new THREE.AxesHelper( 5 );
//scene.add( axesHelper );

var cube; // debugging helper remember to remove this

function main() {
  cube = new Rubiks3d.Cube(scene, TOTAL_CUBE_SIZE, STARTING_CUBE_STRING, CUBE_STARTING_POSITION, CUBE_SPACING_PERCENTAGE, LINE_WIDTH, COLOR_MAPPINGS);
  cube.visible = true;
}
...
function init() {
   ...
   */
  // init scene
  scene = new THREE.Scene(); // init camera

  camera = new THREE.PerspectiveCamera(FIELD_OF_VIEW, window.innerWidth / window.innerHeight, CAMERA_NEAR_CUTOFF, CAMERA_FAR_CUTOFF);
  camera.position.set(...CAMERA_STARTING_POSITION);
  camera.lookAt(new THREE.Vector3(...CAMERA_STARTING_LOOKAT)); // init renderer
  ...
  document.body.appendChild(renderer.domElement); // init OrbitControls

  controls = new THREE.OrbitControls(camera, renderer.domElement);
  controls.target = new THREE.Vector3(...ORBITCONTROLS_TARGET);
  controls.enablePan = false;
  controls.enableZoom = false; // trigger rerender when window resized
...
function animate(time) {
  ...
  TWEEN.update(time); // orbitControls update

  controls.update(); // rerender scene
